...
function Home() {
                ...
                if (editData) {
                    try {
                        ...
                        if (quoteData.items && quoteData.items.length > 0) {
                            const cartItems = quoteData.items.map({
                                "Home.useEffect.cartItems": (item)=>({
                                        product: item.product,
                                        quantity: item.quantity,
                                        unitPrice: item.unitPrice,
                                        salesPrice: item.salesPrice || item.unitPrice
                                    })
                            }["Home.useEffect.cartItems"]);
                            setCart(cartItems);
                        }
                        // Koşulları yükle
                        ...
                        if (quoteData.notes) {
                            ...
                            if (projectDesignMatch || projectDescMatch) {
                                setProjectDetails({
                                    projectDesign: (projectDesignMatch === null || projectDesignMatch === void 0 ? void 0 : projectDesignMatch[1]) || '',
                                    projectDescription: (projectDescMatch === null || projectDescMatch === void 0 ? void 0 : projectDescMatch[1]) || ''
                                });
                            }
                        }
    ...
    const createQuote = async ()=>{
        ...
        try {
            ...
            const itemsWithProductIds = await Promise.all(cart.map(async (item)=>{
                ...
                if (item.isGrouped || productId > 1000000000) {
                    try {
                        const productData = {
                            ...
                            code: item.product.code || "CUSTOM-".concat(Date.now(), "-").concat(Math.random().toString(36).substr(2, 9)),
                        ...
                        if (productResponse.ok) {
                            ...
                            console.log("✅ Manuel/grup ürün kaydedildi: ".concat(productData.name, ", ID: ").concat(productId));
                        } else {
                            const errorData = await productResponse.json();
                            console.error("❌ Ürün kaydedilemedi: ".concat(productData.name), errorData);
            ...
            if (response.ok) {
                ...
                try {
                    var _quoteDetails_items;
                    // Teklif detaylarını al
                    const quoteResponse = await fetch("/api/quotes/".concat(quoteId));
                    if (!quoteResponse.ok) {
                    ...
                    const htmlData = {
                        ...
                        items: ((_quoteDetails_items = quoteDetails.items) === null || _quoteDetails_items === void 0 ? void 0 : _quoteDetails_items.map((item)=>({
                                product: {
                                    name: item.product_name || 'Unknown Product',
                                    brand: item.brand || '',
                                    model: item.model || '',
                    ...
                    alert("Teklif #".concat(quoteId, " başarıyla oluşturuldu ve PDF indiriliyor!"));
                } catch (pdfError) {
                    console.error('PDF oluşturma hatası:', pdfError);
                    alert("Teklif #".concat(quoteId, " oluşturuldu ancak PDF oluşturulamadı."));
                }
            } else {
...
//# sourceMappingURL=src_d5204471._.js.map